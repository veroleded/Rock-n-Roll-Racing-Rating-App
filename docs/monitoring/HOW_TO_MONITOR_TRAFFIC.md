# Как отслеживать трафик приложения

## Быстрый старт

После запуска приложения локально, вы можете отслеживать использование трафика несколькими способами:

### 1. Разовый просмотр (рекомендуется для проверки)

```bash
./scripts/monitoring/monitor-traffic.sh
```

Покажет:

- 📤 Исходящий трафик (например: 2.5 GB)
- 📥 Входящий трафик
- 🔴 Количество активных подписок Redis
- 🤖 Количество переподключений Discord
- ⚠️ Предупреждения о проблемах

### 2. Непрерывный мониторинг (для наблюдения в реальном времени)

```bash
./scripts/monitoring/monitor-traffic-continuous.sh
```

Будет обновлять информацию каждые 30 секунд и показывать:

- Текущий трафик
- Скорость трафика (байт/сек)
- Количество подписок и переподключений
- Автоматические предупреждения

Нажмите `Ctrl+C` для остановки.

### 3. Через браузер

Откройте в браузере:

```
http://localhost:3000/api/metrics
```

Найдите строки:

- `network_bytes_out_total` - общий исходящий трафик в байтах
- `network_bytes_in_total` - общий входящий трафик в байтах

**Пример:**

```
network_bytes_out_total 4611686018427387904
```

Это означает ~4 GB исходящего трафика.

## Что делать при обнаружении высокого трафика?

### Если трафик > 1 GB:

1. **Проверьте подписки Redis:**

   ```bash
   ./scripts/monitoring/monitor-traffic.sh | grep "Redis"
   ```

   - Если подписок больше 5 → возможна утечка подписок
   - Нормально: 1-2 подписки

2. **Проверьте переподключения Discord:**

   ```bash
   ./scripts/monitoring/monitor-traffic.sh | grep "Discord"
   ```

   - Если переподключений больше 10 → возможны проблемы с соединением
   - Нормально: 0-2 переподключения

3. **Остановите приложение:**

   ```bash
   docker-compose -f docker-compose.local.yml down
   ```

4. **Проверьте логи:**
   ```bash
   docker-compose -f docker-compose.local.yml logs --tail=100 | grep -i "error\|warning\|traffic"
   ```

## Примеры использования

### Проверка трафика каждые 5 минут

```bash
watch -n 300 ./scripts/monitoring/monitor-traffic.sh
```

### Мониторинг с записью в файл

```bash
./scripts/monitoring/monitor-traffic-continuous.sh > traffic.log 2>&1
```

### Проверка удаленного сервера

```bash
./scripts/monitoring/monitor-traffic.sh http://your-server:3000/api/metrics
```

## Интерпретация результатов

### Нормальные значения:

- Исходящий трафик: < 100 MB/час (для тестового окружения)
- Подписки Redis: 1-2
- Переподключения Discord: 0-2

### Тревожные значения:

- Исходящий трафик: > 1 GB/час
- Подписки Redis: > 5
- Переподключения Discord: > 10

### Критические значения:

- Исходящий трафик: > 10 GB/час
- Подписки Redis: > 20
- Переподключения Discord: > 50

При критических значениях **немедленно остановите приложение** и проверьте логи!

## История метрик

Метрики автоматически сохраняются в файл каждые 5 минут. Вы можете просмотреть историю:

```bash
# Просмотр последних записей
./scripts/monitoring/view-metrics-history.sh

# Просмотр через API
curl http://localhost:3000/api/metrics/history?limit=20

# Статистика за весь период
curl http://localhost:3000/api/metrics/history?stats=true
```

### Где хранятся метрики

- **Файл:** `logs/metrics/metrics.jsonl`
- **Формат:** JSON Lines (каждая строка - отдельная запись)
- **Интервал сохранения:** Каждые 5 минут
- **Содержимое:** Трафик, подписки Redis, переподключения Discord, полные метрики Prometheus

### Прямой просмотр файла

```bash
# Последние 10 записей
tail -n 10 logs/metrics/metrics.jsonl | jq '.'

# Поиск записей с высоким трафиком
cat logs/metrics/metrics.jsonl | jq 'select(.networkBytesOut > 1073741824)'
```

## Дополнительная информация

Подробнее о метриках см. [METRICS_USAGE.md](./METRICS_USAGE.md)

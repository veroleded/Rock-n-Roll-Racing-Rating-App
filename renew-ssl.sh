#!/bin/bash
# Скрипт для автоматического обновления SSL сертификата
# Используется в systemd timer или cron
#
# ВАЖНО: Перед использованием обновите PROJECT_DIR на ваш реальный путь!

# ============================================
# НАСТРОЙКИ - ИЗМЕНИТЕ НА ВАШИ ЗНАЧЕНИЯ
# ============================================
PROJECT_DIR="/home/ваш_пользователь/projects/discord-bot-new"
COMPOSE_FILE="docker-compose.prod.bogdan.yml"

# ============================================
# ПРОВЕРКИ
# ============================================

# Проверить, что директория существует
if [ ! -d "$PROJECT_DIR" ]; then
    echo "❌ ОШИБКА: Директория проекта не найдена: $PROJECT_DIR"
    echo "   Обновите PROJECT_DIR в скрипте renew-ssl.sh"
    exit 1
fi

# Перейти в директорию проекта
cd "$PROJECT_DIR" || exit 1

# Проверить наличие docker compose
if ! command -v docker &> /dev/null; then
    echo "❌ ОШИБКА: Docker не установлен или не доступен"
    exit 1
fi

# Проверить наличие certbot
if ! command -v certbot &> /dev/null; then
    echo "❌ ОШИБКА: Certbot не установлен"
    exit 1
fi

# ============================================
# ОБНОВЛЕНИЕ СЕРТИФИКАТА
# ============================================

echo "========================================="
echo "🔄 Начало обновления SSL сертификата"
echo "Время: $(date)"
echo "========================================="

# Обновить сертификат
# --quiet: тихий режим (только ошибки)
# --no-random-sleep-on-renew: не ждать случайное время (для автоматизации)
if certbot renew --quiet --no-random-sleep-on-renew; then
    echo "✅ Certbot выполнен успешно"
    
    # Проверить, был ли обновлен сертификат
    # Certbot обновляет только если до истечения < 30 дней
    CERT_INFO=$(sudo certbot certificates 2>/dev/null | grep -A 5 "rocknrollracing.online")
    
    # Перезапустить nginx для применения нового сертификата
    echo "🔄 Перезапуск nginx контейнера..."
    
    # Попробовать docker compose (новая версия)
    if docker compose version &> /dev/null; then
        docker compose -f "$COMPOSE_FILE" restart nginx
    # Иначе использовать docker-compose (старая версия)
    elif command -v docker-compose &> /dev/null; then
        docker-compose -f "$COMPOSE_FILE" restart nginx
    else
        echo "❌ ОШИБКА: docker compose или docker-compose не найдены"
        exit 1
    fi
    
    if [ $? -eq 0 ]; then
        echo "✅ Nginx успешно перезапущен"
        echo "✅ Обновление завершено успешно"
        echo ""
        echo "📋 Информация о сертификате:"
        sudo certbot certificates 2>/dev/null | grep -A 10 "rocknrollracing.online" || echo "Не удалось получить информацию"
    else
        echo "❌ ОШИБКА: Не удалось перезапустить nginx"
        echo "   Попробуйте вручную: docker compose -f $COMPOSE_FILE restart nginx"
        exit 1
    fi
else
    EXIT_CODE=$?
    echo "⚠️  Certbot завершился с кодом: $EXIT_CODE"
    
    # Код 0 означает, что обновление не требуется (сертификат еще действителен)
    if [ $EXIT_CODE -eq 0 ]; then
        echo "ℹ️  Сертификат не требует обновления (еще действителен более 30 дней)"
    else
        echo "❌ ОШИБКА при обновлении сертификата"
        echo ""
        echo "📋 Текущий статус сертификата:"
        sudo certbot certificates 2>/dev/null | grep -A 10 "rocknrollracing.online" || echo "Не удалось получить информацию"
        exit 1
    fi
fi

echo ""
echo "Время завершения: $(date)"
echo "========================================="
echo ""

